...
const Astar = {
    ...
    astar(map, heuristic, cutCorners) {
        var listOpen = [];
        var listClosed = [];
        var listPath = [];
    ...
    addListToList(listA, listB) {
        for (x in listA)
            listB.push(listA[x]);
    },

    removeMatchingNodes(listToCheck, listToClean) {
    ...
    cullUnwantedNodes(listToCull, listToCompare) {
    ...
    returnNodeWithLowestFScore(list) {
    ...
    createTerminalNode(map, heuristic, nodeType, nodeGoal) {
    ...
    returnHScore(node, heuristic, nodeGoal) {
    ...
    NodeCoordinate(row, col) {
        this.row = row;
        this.col = col;
    },

    Node(row, col, map, heuristic, parentNode, nodeGoal) {
